...
use nexofolio_common::{Error, ProjectId, Result, Secret, UserId};
...
impl PlatformAccess for PostgresAccess {
    ...
    async fn create_environment(
        ...
        project: ProjectId,
        name: &str,
    ) -> Result<nexofolio_access_contracts::Environment> {
        self.require_project(p, project).await?;
        let mut tx = self.database.pool.begin().await.map_err(db_error)?;
        ...
        let result = crate::environments::resolve(
            &mut tx,
            pid(project),
            &nexofolio_access_contracts::EnvironmentRef::ByName(
                nexofolio_access_contracts::EnvironmentByName { name: name.into() },
            ),
        )
        .await?;
        tx.commit().await.map_err(db_error)?;
