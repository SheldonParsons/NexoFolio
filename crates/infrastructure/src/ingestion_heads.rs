...
use nexofolio_contracts::{Error, Result, STRUCTURE_ALGORITHM};
...
pub(crate) async fn load(
    ...
    let previous=sqlx::query(r#"SELECT h.identity_key,h.structural_hash,h.algorithm_version,h.ingestion_id,h.structural_projection,CASE WHEN h.algorithm_version <> $4 THEN i.raw_record ELSE NULL END AS legacy_raw
            FROM ingestion_heads h
            JOIN ingestion_inbox i ON i.id=h.ingestion_id
            WHERE h.project_id=$1
            AND h.environment_id=$2
            AND h.identity_key=ANY($3)"#).bind(project).bind(environment_id).bind(identities).bind(STRUCTURE_ALGORITHM).fetch_all(&mut **tx).await.map_err(db_error)?;
    ...
    for r in previous {
        ...
        if r.get::<String, _>("algorithm_version") != STRUCTURE_ALGORITHM {
            ...
            sqlx::query("UPDATE ingestion_heads SET algorithm_version=$6,structural_projection=$4,structural_hash=$5 WHERE project_id=$1 AND environment_id=$2 AND identity_key=$3")
                    .bind(project).bind(environment_id).bind(&identity).bind(&head.projection).bind(&head.hash).bind(STRUCTURE_ALGORITHM).execute(&mut **tx).await.map_err(db_error)?;
        }
